...
| 维度 | Excel/老师傅手工 | **APS Engine（本插件）** | 商业 APS（SAP/Oracle…） | frePPLe |
|---|---|---|---|---|
| 成本 | 0（但人力高） | **0（MIT 开源）** | 几十万~百万/年 | 0（开源） |
| 求解器 | 老师傅经验 | **CP-SAT + 换型 2-opt + JSSP** | 专有求解器 | 启发式/约束 |
| 可复现 | ✗ | **✓（seed=42）** | ✓ | ✓ |
| 多工序/前序 | 靠人记 | **✓ solve_jssp** | ✓ | ✓ |
| 设备直连（Modbus/DNC） | ✗ | **✓** | 部分（需二次开发） | ✗ |
| 多通道（微信/钉钉/企微） | ✗ | **✓** | ✗/需集成 | ✗ |
| 中文/国内设备适配 | 天然 | **✓ 天然（温州/佛山机器）** | 需实施顾问 | 英文 |
| 上手到出第一版排产 | — | **小时级（三张表）** | 月级 | 天级 |
